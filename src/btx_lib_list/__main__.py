...
Purpose
-------
Provide the ``python -m btx_lib_list`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`btx_lib_list.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Why
        Implements ``python -m btx_lib_list`` by delegating to the
